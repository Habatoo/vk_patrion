...
@app.route('/api/create_user/<vk_id>/<creator>', methods=['GET', 'POST'])
def get_create_user(vk_id, creator=None):
    # create user
    def add_folder():
    ...
    if user and not creator: 
        return make_response(jsonify({'error': 'User already exists'}), 412)
    
    if creator and user: # new user-creator
        user.creator =  True
        db.session.commit()
        add_folder()
        return jsonify({'user': [{'vk_id': user.vk_id}]})

    if creator and not user: # make user-creator
        user = User(
            vk_id=vk_id,
            creator = True,
            )
        add_folder()
    else: # make simple user
        user = User(
            vk_id=vk_id,
            )
    db.session.add(user)
